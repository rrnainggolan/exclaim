...
class ExpenseClaimRejected extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        if($this->target == 'user') {
            $message->subject('Your expense claim has been rejected')
            ->greeting('Hello')
            ...
            $message->subject('Expense claim has been rejected')
            ->greeting('Hello')
            ->line('Expense Claim with code: '.$expenseClaimCode.' has been rejected.')
            ->action('Click here to view the request', route('expense-claims.show', $this->expenseClaim->id))
            ->line('Thank you for using our application!');
        }
